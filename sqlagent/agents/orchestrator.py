...
class OraOrchestrator:
    ...
    async def run(self, state: QueryState) -> dict:
        ...
        # Inject database dialect so SQL Agent generates compatible syntax
        dialects = set()
        for conn in self._services.connectors.values():
            dialects.add(getattr(conn, 'dialect', 'sql'))
        if dialects:
            query_spec += f"\n\nDATABASE DIALECT: {', '.join(dialects)}. Generate SQL compatible with this dialect.\n"
        ...
        for attempt in range(MAX_ATTEMPTS):
            ...
            if execution_error:
                ...
                dialect_info = f"\nDATABASE DIALECT: {', '.join(dialects)}.\n" if dialects else ""
                query_spec = (
